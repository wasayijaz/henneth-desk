...
function main() {
  ...
  assert(liveMariMechanism.status === "blocked" && liveMariMechanism.reason, "live MARI E&P mechanism remains explicitly blocked");
  assert(/financial truth is not_qualified/.test(liveMariMechanism.reason) && /Peshawar Block filings do not provide/.test(liveMariMechanism.reason), "live MARI E&P mechanism retains its emitted evidence blockers");
  assert(/deterministic EBITDA\/FCF operand lineage/.test(liveMariMechanism.reason) && /no unresolved canonical conflicts/.test(liveMariMechanism.reason), "live MARI E&P mechanism retains formal-output activation requirements");
