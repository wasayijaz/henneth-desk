...
# Conservative bounds keep products finite while accommodating PSX-sized inputs.
MAX_ABS_NUMBER = 1.0e100
MAX_INPUT_FIELDS = 64
MAX_SPEND_ROWS = 256
...
_POSITIVE_FIELDS = ("initial_production_boe_pd", "oil_price_usd_bbl", "gas_price_usd_mmbtu", "gas_mmbtu_per_boe", "fx_pkr_usd", "shares_out")
...
def validate_case(case: Mapping[str, Any]) -> list[str]:
    ...
    for field in sorted(inputs):
        ...
        elif field in _NON_NEGATIVE_FIELDS or field == "market_gap_pkr":
            number = _finite(value)
            if number is None:
                violations.append(f"{field}: must be a finite number (booleans are not numbers)")
            elif field in _NON_NEGATIVE_FIELDS and number < 0:
                violations.append(f"{field}: must be >= 0")
