...
| Financial-truth qualification | `scripts/build_financial_truth_qualification.py` + `financial_truth_qualification.py` -> `state/company_intel/financial_truth_qualification.json` | strict retained-evidence scorecard ranks a leading candidate without making it golden; requires five annual income triplets, eight qualified reported-quarter fact sets, five annual operating-cash-flow periods, an official share-count tie-out and no unresolved conflicts before a financial-truth claim; document metadata remains separately labelled as non-qualifying coverage |
