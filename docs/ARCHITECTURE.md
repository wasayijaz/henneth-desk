...
| Historical State Map | `scripts/build_historical_state_map.py` -> `state/company_intel/historical_state_map.json` | deterministic, strictly pre-event historical context mapping for observed Intelligence Cases over retained event studies, conditional benchmarks, pre-event price setups, and financial truth qualification; descriptive past context only, with strict sample gating (N >= 3) and no forecast/valuation feed |
...
| OCR quarantine candidates | `scripts/ocr_quarantine.py` -> optional append-only audit ledger | hash-bound local official PDF/page OCR evidence only; emits page text candidates with engine, confidence, geometry and render receipts, all `audit_only`/quarantined, and cannot create parser receipts, financial facts, financial truth, forecasts, valuations or market expectations |
| Official share-capital candidates | `scripts/share_capital.py` via `document_intelligence.py` -> `state/company_intel/official_share_capital_candidates.json` | source-bound, arithmetic-tied capital-note evidence captured only during verified document processing; candidates retain the original document hash and page but remain unapproved review records that cannot activate financial truth or any formal engine |
...
| Financial-truth qualification | `scripts/build_financial_truth_qualification.py` + `financial_truth_qualification.py` -> `state/company_intel/financial_truth_qualification.json` | strict retained-evidence scorecard ranks a leading candidate without making it golden; formal activation requires five annual and eight direct reported-quarter full-statement schedules (income, balance sheet, cash flow), deterministic source-bound EBITDA/FCF operand lineage, an official share-count tie-out, and no unresolved canonical conflicts. Legacy three-line readiness is visibility-only. Document metadata remains separately labelled as non-qualifying coverage. |
| Owner-gated official share-capital tie-outs | `config/ci_share_capital_approval_manifest.json` + `scripts/build_official_share_capital_approvals.py` -> `state/company_intel/official_share_capital_approvals.json` | empty-by-default, source-bound bridge from a named tied-out candidate to an eligible reported share-count fact. An approval may name only an existing candidate and approval date; values remain in the official candidate, a successful matching reprocess receipt/hash is mandatory, and approval alone never activates formal engines. |
...
| MLCF/PIOC event-model readiness manifest | `scripts/build_mlcf_pioc_readiness_manifest.py` -> `state/company_intel/mlcf_pioc_readiness_manifest.json`; `scripts/mlcf_pioc_case_run_adapter.py` | compact read-only blocker/input checklist for the retained MLCF acquisition-control case; preserves official refs and known deal fields, records PIOC outside the CI pilot, and hard-blocks formal output without a generic economic engine. Case runs rederive this manifest in-process from current producer authorities and the same IntelligenceCase build; the checked-in JSON is generated output, never authority |
...
| MARI material-event intake | `config/ci_mari_sales_event_intake_allowlist.json` + `reprocess_company_documents.py --mari-sales-event-intake` | exact two-document PSX-only boundary for the Sky47 launch evidence; source/hash-bound material information can create a reported operating event but is prevented from creating financial facts, qualification coverage, or formal-engine inputs |
