...
| Financial truth qualification | `financial_truth_qualification.json`; `financial_reprocess_blockers.json` | DGKC leads on retained annual-income coverage but is not golden; MLCF is the selected industrial case | MLCF remains at 3/5 annual income triplets, 2/5 consolidated annual OCF periods and 0/8 qualified direct consolidated quarters; its official share-count/capital-note tie-out is passed. Four source-bound derived facts now provide FY2024/FY2025 D&A and EBITDA lineage from `psx:260032`, including exact note operands, formula version, geometry and ROU treatment. The authoritative gate remains stricter: 0/5 annual and 0/8 quarterly full-statement schedules because the 23 direct schedule facts and deterministic FCF lineage are not yet canonically admitted. Formal outputs remain fail-closed. |
...
| Intelligence Case lifecycle | `state/company_intel/intelligence_cases.json` | 4 `Observed`, 0 `Corroborated`, 0 `Modelled`, 0 `Validated`, 0 `Published`, 0 `Monitoring`, 0 `Closed` | MLCF/PIOC, MARI Peshawar, MARI Sky47, and PSO distribution expansion are source-bound observations only. The final three-company shape now has an E&P lane, an industrial lane, and a distinct sales-led PSO lane, but none may activate forecast, valuation, expectations, recommendation, or publication. |
...
| C — sales-led | sales hiring, geography, branch/channel, product-sales infrastructure | PSO selected; CNERGY and MARI Sky47 retained as non-counting leads | Observed | PSO now has a distinct official-source `Observed` distribution-network expansion seed from `psx:260771`: page 15 reports 107 gross new FY2025 outlets and page 310 binds the network counts at 3,649 in FY2025 versus 3,580 in FY2024. The 69 net active change is derived, and the 38-outlet difference remains explicitly unresolved rather than inferred as closures. PSO is not Corroborated, Modelled, Validated or Published: it lacks independent-originator corroboration, model-ready financial truth, and source-qualified outlet economics. |
...
| 2026-09-20 | Added the first canonical-safe derived-financial-fact interface for MLCF FY2024/FY2025 D&A and EBITDA. | Four `derived_fact` records from hash-bound `psx:260032` now carry exact reported inputs, pages, geometry, formula and calculation-version lineage. Adversarial checks reject source, page, value, formula, missing-input and epistemic-type forgeries; reconciliation accepts only the validated bounded lane. This does not promote the 23 direct schedule candidates: MLCF remains 0/5 annual full schedules, 0/8 quarterly full schedules, and every formal engine remains blocked. |
...
| 2026-08-31 | Completed independent table and interim schedule audits for retained MLCF FY2022 evidence. | The annual table audit independently retained one strict Sales-net candidate and rejected PAT/EPS/OCF as geometrically ambiguous; the broader annual schedule retains 16 audit-only candidates with nine explicit omissions. The three interim reports record direct-quarter geometry where visible, but yield zero qualified sets: all lack official availability dates, PAT owner attribution is not proved, and two EPS cells are non-unique. No fact, coverage, case, or formal-output state changed. |
| 2026-08-31 | Audited the highest-leverage retained MLCF FY2025 annual full-schedule path. | Official PSX identity, hash and availability are retained for `psx:260032`, but its exact local PDF bytes are absent from the approved raw path. The audit records 25 explicit missing schedule slots across the expected consolidated balance-sheet, income-statement and cash-flow pages; it cannot use prior snippets as replacement geometry. No fact, coverage, case or formal output changed. |
| 2026-08-31 | Removed a stale MLCF case-envelope counter freeze. | The observed-case projection now validates the authoritative financial-truth counters dynamically rather than requiring historical present counts and period lists. Strict 5/8/5 thresholds, share-source shape, tie-out state and fail-closed formal-output boundaries remain enforced; synthetic qualified truth can proceed only to the owner-input boundary, never to a computed output or unevidenced case promotion. |
| 2026-08-31 | Completed a bounded retained-report geometry review for MARI FY26 Q1–Q3. | Official `psx:264550`, `psx:271327`, and `psx:275583` retain direct-three-month income text, but none provides a safely bound consolidated direct-quarter Revenue/PAT/EPS set under the current header, basis, unit and duration rules. Q2/Q3 OCF facts are cumulative (6M/9M), not direct quarters. This is a zero financial-truth delta: MARI remains 0/5 annuals, 0/8 direct quarters, with no share-count tie-out and all formal outputs blocked. |
| 2026-08-31 | Closed the bounded distinct sales-led candidate review without promotion. | CNERGY's primary issuer sales-record source remains unavailable at the exact URL; its retained alternative `issuer:439f94b15635143d5d16b663` is source-bound observed evidence only and CNERGY has zero load-bearing financial coverage. PSO, GAL and BOP are weaker retained lanes. MARI Sky47 cannot count twice, so the distinct sales-led golden slot remains unselected pending an owner-approved evidence path. |
| 2026-08-31 | Ran the approved MARI FY26 Q1–Q3 exact-ID restage once. | The run left no canonical MARI fact, receipt or coverage change. Retained Q2/Q3 raw bytes/page geometry are unavailable, and the strict parser cannot bind their mixed direct/cumulative tables from legacy snippets. MARI remains 0/5 annual, 0/8 direct-quarter and share-count-missing; all formal outputs remain blocked. |
| 2026-08-31 | Tightened the fail-closed parser for a next-calendar-year filing with a prior-year wrapped statement header. | A shifted `2025/2024` header may now be recognized only when the exact matching statement period is immediately above that same table, with existing duration, unit, scale, no-future-year and local consolidated-basis gates intact. Focused fixtures reject missing basis, an incompatible period, and a matching date elsewhere on the page. This parser correction alone promotes no MARI facts or coverage: an actual direct-quarter Revenue/PAT/EPS set still requires all three source columns to be geometrically bound. |
| 2026-09-01 | Added the MLCF/PIOC Observed mechanism panel to the investor case route. | The panel source-links only the 18 December 2025 MLCF public-offer/control filing (`psx:267429` p.3) and the 28 April 2026 MLCF filing reporting PIOC dispatches included in local-market totals after the February acquisition (`psx:275425` p.4). It explicitly excludes standalone PIOC capacity, volume, revenue, margin, EPS, debt, cash flow, synergies, forecasts and financial-model activation. This completes the reported event-to-operating-linkage step only; lifecycle remains `Observed` and formal outputs remain blocked. |
| 2026-09-01 | Made both active MARI mechanisms fail closed at the case boundary. | The Peshawar E&P and Sky47 sales-led cases retain their dated official observations but render their mechanism sections as blocked until MARI financial truth and source-qualified project inputs exist. Focused case and payload contracts cover both cases; no lifecycle, financial fact, forecast, valuation, or expectation output changed. |
| 2026-09-01 | Added an evidence-bound qualitative mechanism path to Ask Henneth. | Ask may project an available reported-fact mechanism only when its item text is non-numeric and its official evidence has an HTTPS source URL, document ID, immutable content hash, and one-based page. It emits server-owned citations and a fixed non-activation boundary; blocked MARI mechanisms project as null. The adversarial Ask contract proves this path cannot activate financial impact, valuation, or expectations. |
| 2026-09-01 | Added the MARI/Peshawar Observed mechanism panel to the investor case route. | The panel source-links only MARI's 30 September 2025 acquisition/operator filing (`psx:260446` p.1) and its 27 February 2026 filing reporting 65% working interest with operatorship (`psx:271327` p.6). It explicitly excludes reserves, discovery, production, well cost, timing, capex, commodity exposure, project economics, forecasts and financial-model activation. This completes the reported E&P operating-linkage step only; lifecycle remains `Observed` and formal outputs remain blocked. |
| 2026-08-31 | Ran the approved MLCF FY24 Q1–Q3 exact-ID restage once. | Exact official IDs `psx:219092`, `psx:225623`, and `psx:229941` are metadata/hash-bound but have no local retained originals; each failed closed as `retained_original_not_approved` with zero bytes/pages, receipts, facts or coverage change. Do not retry this tranche without a verified transport/retention change. |
| 2026-09-01 | Re-audited the first industrial/cement selection against the Alpha’s actual-expansion requirement. | DGKC’s issuer-hosted FY2025 Corporate Briefing (`issuer:c7230a44854c74a77777465a`, p.9, hash `df7976…4685c`) reports a 90-million-bag NPL PP-bag plant commissioned in Q3 FY25, making it the strongest retained scope fit. It remains unselected: the issuer record has no source publication/event timestamp (only quarter-level timing), so no point-in-time-safe Observed case or model may be created. MLCF remains the only dated industrial Observed case, but it is acquisition/control rather than an expansion. |
| 2026-09-01 | Made the rejected DGKC commissioning lead explicit in the case-product contract. | The retained PP-bag commissioning event now exposes its exact missing-date rejection rather than a generic no-case result. A deterministic seed check proves it remains outside `Observed` with no inferred date, lifecycle promotion, forecast, valuation, or expectations output. |
| 2026-09-01 | Added a distinct-golden-lane acceptance guard. | A Published Alpha set must contain exactly one E&P, industrial/cement, and sales-led case, with three distinct tickers and case IDs. MARI Sky47 cannot be reused beside MARI’s E&P case; CNERGY’s retained lead remains unpromoted. A ticker-agnostic future fixture preserves the path for a genuinely qualifying sales-led company. |
| 2026-09-01 | Refreshed the financial-series quarantine against the current parser revision. | No reported value changed. Rows extracted under legacy `block_geometry_v5` are now correctly `audit_only` with an explicit quarantine flag instead of stale `model_loadable`; the model-inputs contract now passes and all real formal outputs remain blocked. |
| 2026-09-01 | Reconciled the MARI E&P readiness check with the current retained evidence classification. | The checker now recognises audit-only cumulative cash-flow records as explicitly not eligible for financial truth rather than expecting no quarterly evidence at all. MARI remains 0/5 annual, 0/8 direct quarters, missing share-count evidence, and blocked from model activation. |
| 2026-09-01 | Recovered and reclassified CNERGY's previously unavailable issuer sales-record PDF. | The exact issuer URL now yields hash-bound page-1 evidence for April 2018 sales and throughput records. It remains outside the sales-led case lane because it reports performance, not a sales-team, channel, geography, branch, or product-sales-infrastructure expansion. No case, financial fact, qualification, forecast, valuation, or expectation output changed. |
| 2026-09-01 | Added the read-only Event-to-Value Product Readiness dashboard and fail-closed checker. | `state/company_intel/event_to_value_product_readiness.json` now summarizes the exact three-company / three-case / nine-scenario Alpha denominator, formal live-output counts, Ask test status, thesis-monitoring activity, required nulls, provenance, no-lookahead and production-gate status from retained state only. Implemented engines remain distinct from computed outputs; missing or invalid evidence stays blocked and is projected into the private CI slice. |
| 2026-09-01 | Defined the deterministic readiness/integrity rebuild order. | Run readiness builder, integrity finalizer, readiness builder again, then integrity finalizer again (same UTC cutoff and source SHA). The second pass is byte-identical: readiness is hashed but not metadata-stamped by the integrity finalizer, avoiding a self-referential loop while the checker still detects tampering against the manifest hash. |
| 2026-09-13 | Promoted PSO's FY2025 distribution-network expansion into the canonical Observed layer. | Official PSX annual report `psx:260771` is bound to URL `https://dps.psx.com.pk/download/document/260771.pdf`, 13,034,584 bytes, 420 pages, SHA-256 `c336ba824d4010dff3878779c31bbc76fd85ab6704b4ecb4837de821f9bd0a2d`, and exact pages 15 and 310. The case records 107 gross new FY2025 outlets, 3,649 FY2025 network outlets, 3,580 FY2024 network outlets, a derived 69 net active outlet change, and a 38-outlet unresolved difference that is not inferred as closures. This selects PSO as the distinct sales-led Observed lane only; PSO remains blocked by missing independent corroboration, red financial truth, and missing source-qualified outlet economics, so no forecast, valuation, market-expectations output, publication, or recommendation is activated. |
| 2026-09-13 | Made the selected MLCF cement operating-history blocker actionable instead of cryptic. | `state/company_intel/cement_operating_series.json` now reports exactly why MLCF cannot feed the cement model: 0/5 aligned annual cement operating rows and 0/2 event-specific PIOC bridge periods. The artifact lists the required official evidence, required metrics and next evidence actions, while preserving all downstream statuses as `not_activated`. This improves the CI tile/readiness explanation only; no audit-only dispatch context was promoted, no financial-truth count changed, and no forecast, valuation, market-expectations output or publication was activated. |
| 2026-09-13 | Extended the fail-closed PSX statement parser against exact retained MLCF and PSO filings. | Guarded descriptor/unit-band normalization, split point-in-time balance-sheet headers and page-local long-row association increase the exact MLCF FY2025 annual output from 8 to 24 model-loadable candidates and expose 24 consolidated PSO Q3 candidates, including complete current and comparative direct-quarter Revenue/PAT/EPS sets. Adversarial checks reject missing consolidated basis, detached or competing headers, cross-statement capture and adjacent-row value borrowing. These are review candidates only: no canonical fact, qualification count, forecast, valuation, expectation or lifecycle state changed. PSO promotion remains explicitly owner-gated. |
| 2026-09-13 | Added Past Context / Historical State Map as a first-class Part 5 Alpha product and shipped its first real case UI. | Four deterministic `historical_context/not_forecast` envelopes now bind the selected MLCF, MARI and PSO cases to exact canonical events or document/hash evidence, pre-information market state, financial-truth status and strict analogue availability. The CI route renders case state, binding/cutoff provenance, individual horizon outcomes and honest no-analogue states across desktop/mobile contracts; similarity remains separate from evidence trust and cannot activate scenarios, forecasts, valuation, expectations or lifecycle promotion. |
| 2026-09-13 | Corrected event-study and Past Context market cutoffs to use public information availability rather than operating timing. | PSO's network evidence preserves its FY2025 effective date of 2025-06-30 but anchors market context to the 2025-10-02 publication, using the strictly prior 2025-10-01 session. The same rule now covers future-effective announcements: published/detected/available dates govern market-study cutoffs whenever present, while the operating effective date remains separately visible. Global and focused no-lookahead regressions reject baselines on or after information availability. |
| 2026-09-13 | Audited retained analogue availability for all four active Alpha case envelopes without weakening the sample gate. | Every case currently has zero mature independent Tier 1 prior analogues, so all 1Q/2Q/4Q/8Q distributions and scenario calibration remain suppressed. Broad cross-sector acquisition matches were rejected as non-comparable rather than used for MLCF. Readiness requires at least three mature exact outcomes per horizon, not merely three candidate event records. |
| 2026-09-13 | Proved the exact retained MLCF FY2025 consolidated balance-sheet continuation and widened its fail-closed financial-series eligibility. | Hash-pinned pages 291–295 of `psx:260032` now yield 40 model-loadable facts: 20 balance-sheet, 12 income-statement and 8 cash-flow facts across FY2025/FY2024. Exact page adjacency, statement basis, periods, units, duplicate suppression and source hash are checked; unsupported or mismatched rows remain audit-only. Canonical state has not yet been restaged, so MLCF remains red and Part 1 remains incomplete. |
