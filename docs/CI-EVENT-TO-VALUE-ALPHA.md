...
| Intelligence Case lifecycle | `state/company_intel/intelligence_cases.json` | 2 `Observed`, 0 `Corroborated`, 0 `Modelled`, 0 `Validated`, 0 `Published` | MLCF/PIOC and MARI are source-bound observed seeds only. Neither may activate forecast, valuation, expectations, recommendation, or publication. |
...
| 7 | Intelligence Case interface | In progress | The private route renders the two `Observed` source-grounded seeds; the MLCF/PIOC page now projects its exact source-bound confidence dimensions and confirmation/break/watch checks. | This is a read-only evidence context, not a model output. A case remains blocked from formal financial impact, scenarios, valuation, expectations, or publication until the relevant gates qualify. |
