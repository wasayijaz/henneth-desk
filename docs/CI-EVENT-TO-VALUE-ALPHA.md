...
| 2026-08-31 | Retried the already-approved local-only MLCF annual/Q3 parser sources without network fallback. | `psx:260032` and `psx:275425` are still hash-bound in retained metadata, but both named local raw-PDF paths are absent. The exact-ID transport returned `retained_original_unavailable` for each, wrote no fact or receipt, and did not download, OCR, substitute a source, or change coverage. Restoring the exact retained bytes is the next evidence-path blocker. |
