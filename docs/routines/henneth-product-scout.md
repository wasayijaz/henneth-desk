...
Sunday product-improvement scout. It proposes and ranks; it never builds, edits source, or changes
...
3. Run `python scripts/provenance_lint.py` and inspect Room QA caveats. Then run one Product Scout
   ...
   from this card. Produce at most ten items across exactly `accuracy`, `clarity`, and `feature`.
4. Every item must cite a concrete observation, include an `existing_check` naming the page function
   ...
   Carry forward open items without duplicating them and sort the full list by score.
...
`qa_observations[]`; `new_items[]`; `carried_items[]`; `categories`; `item_count` (≤10);
